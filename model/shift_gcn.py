...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3):
        ...
        self.Linear_bias = nn.Parameter(torch.zeros(
            1, 1, out_channels, requires_grad=True, device='cuda'), requires_grad=True)
        nn.init.constant(self.Linear_bias, 0)

        # self.Feature_Mask = nn.Parameter(torch.ones(
        #     1, 25, in_channels, requires_grad=True, device='cuda'), requires_grad=True)
        ...
        for i in range(25):
            for j in range(in_channels):
                index_array[i*in_channels +
                            j] = (i*in_channels + j + j*in_channels) % (in_channels*25)
        self.shift_in = nn.Parameter(
            torch.from_numpy(index_array), requires_grad=False)

        index_array = np.empty(25*out_channels).astype(np.int)
        for i in range(25):
            for j in range(out_channels):
                index_array[i*out_channels +
                            j] = (i*out_channels + j - j*out_channels) % (out_channels*25)
    ...
    def forward(self, x0):
        ...
        x = torch.einsum('nwc,cd->nwd', (x, self.Linear_weight)
                         ).contiguous()  # nt,v,c
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True):
        ...
        if not residual:
            ...
            self.residual = tcn(in_channels, out_channels,
